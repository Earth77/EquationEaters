...
public class Font extends Sprite {
    ...
    public void draw(int renderingProgram, String stringToRender, Vector2f positionIn) {
        ...
        for (int i = 0; i < characterArray.length; i++) {
            ...
            tempPosition += characterWidth + characterSpacing;
        }

    }

    public void draw(int renderingProgram, String stringToRender, Vector2f positionIn, int widthIn, int heightIn) {
        ...
        for (int i = 0; i < characterArray.length; i++) {
            ...
            int toAsciiAndBack;
            float[] texCoords;
            //       System.out.println("Character to check : " + charToRender);
            if (Character.isLetter(charToRender)) {
                toAsciiAndBack = (int)charToRender - 65;
//        System.out.println("to ascii and back - 65");
                texCoords = alphabetArray.get(toAsciiAndBack).getCoords();
            } else if (Character.isWhitespace(charToRender)) {
                //      System.out.println("is white space");
                texCoords = alphabetArray.get(26).getCoords();
            } else if (Character.isLetter('{')) {
                //         System.out.println("Bracket found");
                texCoords = alphabetArray.get(27).getCoords();
            } else {
                //         System.out.println("To ascii and back 48");
                toAsciiAndBack = (int)charToRender - 48;
                if (charToRender == '{') {
                    toAsciiAndBack = 10;
                }
            ...
            if (!reflectedX) {
                GLES30.glUniform1f(GLES30.glGetUniformLocation(renderingProgram, "width"), widthIn);
            } else {
                GLES30.glUniform1f(GLES30.glGetUniformLocation(renderingProgram, "width"), -widthIn);
            }

            if (!reflectedY) {
                GLES30.glUniform1f(GLES30.glGetUniformLocation(renderingProgram, "height"), heightIn);
            } else {
                GLES30.glUniform1f(GLES30.glGetUniformLocation(renderingProgram, "height"), -heightIn);
            }
            ...
            tempPosition += widthIn + characterSpacing;
        }

    }

    public void drawGrey(int renderingProgram, String stringToRender, Vector2f positionIn, int widthIn, int heightIn) {
...
//        System.out.println("to ascii and back - 65");
            ...
            GLES30.glUniform1i(GLES30.glGetUniformLocation(renderingProgram, "opacity"),101);
            GLES30.glUniform1i(GLES30.glGetUniformLocation(renderingProgram, "textureToUse"), textureID);
            GLES30.glUniform1i(GLES30.glGetUniformLocation(renderingProgram, "clrReverse"), 1);
    ...
    public void drawNoCenter(int renderingProgram, String stringToRender, Vector2f positionIn, int widthIn, int heightIn) {
        ...
        for (int i = 0; i < characterArray.length; i++) {
            ...
            int toAsciiAndBack;
            float[] texCoords = numberArray.get(1).getCoords();
            //       System.out.println("Character to check : " + charToRender);
            if (Character.isLetter(charToRender)) {
...
//        System.out.println("to ascii and back - 65");
                ...
                //      System.out.println("is white space");
                texCoords = alphabetArray.get(26).getCoords();
            } else if (charToRender == '?') {
                texCoords = numberArray.get(11).getCoords();
            } else if (charToRender == ',') {
                texCoords = numberArray.get(13).getCoords();
            } else if (charToRender == '.'){
                texCoords = numberArray.get(12).getCoords();
            } else if (charToRender == '\'') {
                texCoords = numberArray.get(14).getCoords();
            } else {
            ...
            GLES30.glUniform1f(GLES30.glGetUniformLocation(renderingProgram, "positionx"), tempPosition);
            GLES30.glUniform1f(GLES30.glGetUniformLocation(renderingProgram, "positiony"), position.y);
            GLES30.glUniform1f(GLES30.glGetUniformLocation(renderingProgram, "zdepth"), -0.03f);
    ...
    public void fillFontArrays() {
        ...
        for (int j = 1; j < 3; j++) {
            for (int l = 0; l < 8; l++) {
                ...
                float bl2 = ((64 + characterHeight * j - characterHeight) * pixelCoord);
